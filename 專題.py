...
time.sleep(10)
laptop = crawler()
laptop.search_items('筆電')
#laptop.show_items()
time.sleep(10)
computer = crawler()
computer.search_items('電腦')

a = database('cellphone')
print(a.db_name)
a.conn = sqlite3.connect(a.db_name)
a.cur = a.conn.cursor()         
a.insert_items(cellphone.items, 'new')
a.conn.close()

print('Database disconnect')
lap = database('laptop')
print(lap.db_name)
lap.conn = sqlite3.connect(lap.db_name)
lap.cur = lap.conn.cursor()         
lap.insert_items(laptop.items, 'new')
lap.conn.close()

print('Database disconnect')
com = database('computer')
print(com.db_name)
com.conn = sqlite3.connect(com.db_name)
com.cur = com.conn.cursor()         
com.insert_items(computer.items, 'new')
com.conn.close()
print('Database disconnect')
...
class database():
    ...
    def __init__(self, name):
        try:      
            self.db_name = 'C:\\Users\\User\\Desktop\\專題\\sqlite3\\' + name + '.sqlite'
...
#           print("database connect close")
    ...
    def insert_items(self, items, table): 
        self.cur.execute( 'select * from new order by price')
